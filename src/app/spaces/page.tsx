      ...
      <main className="mx-auto w-full max-w-lg px-4 pb-12 pt-4 sm:pb-14" dir="rtl">
        <header className="mb-5 overflow-hidden rounded-3xl border border-slate-200/80 bg-gradient-to-br from-indigo-600 via-violet-700 to-slate-900 p-5 text-white shadow-lg ring-1 ring-inset ring-white/20">
          ...
          <div className="mt-4 grid grid-cols-1 gap-2 sm:grid-cols-2">
            ...
            <button type="button" onClick={openEdit} className="rounded-2xl border border-white/35 bg-white/10 px-4 py-2.5 text-xs font-extrabold text-white transition hover:bg-white/20">
              تنظیم علاقه‌مندی‌ها
            </button>
          </div>
        </header>
            ...
            <section className={SECTION_CARD}>
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                {USER_SPACE_KEYS.map((k) => (
                  <button key={k} type="button" onClick={openEdit} className={`rounded-2xl px-2.5 py-2.5 text-xs font-bold ring-1 transition ${
                    preferredSet.has(k)
