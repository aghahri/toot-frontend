...
type Row = { id: string; name: string; type: string; networkId: string | null; isFeatured: boolean };
...
export default function AdminGroupsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await apiFetch<PageData>(`admin/groups?${qs}`, { method: 'GET', token });
      setData(res);
    } catch (e) {
            ...
            <tbody>
              {data.data.map((g) => (
                <tr key={g.id} className="border-t border-slate-800">
                  ...
                  <td className="px-3 py-2 font-mono text-xs">{g.networkId ?? '—'}</td>
                  <td className="px-3 py-2">{g.isFeatured ? 'yes' : 'no'}</td>
                  <td className="px-3 py-2 text-right">
                    <Link href={`/admin/groups/${g.id}`} className="text-sky-400 hover:underline">
                      Edit
                    </Link>
                  </td>
                </tr>
