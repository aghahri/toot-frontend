...
export default function AdminChannelDetailPage() {
  ...
  const [row, setRow] = useState<C | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (!token) return;
      try {
        const ch = await apiFetch<C>(`admin/channels/${encodeURIComponent(id)}`, { method: 'GET', token });
        if (c) return;
        setRow(ch);
        setName(ch.name);
        setDescription(ch.description ?? '');
        setIsFeatured(ch.isFeatured);
      } catch (e) {
  ...
  async function save() {
    ...
    setErr(null);
    try {
      await apiFetch(`admin/channels/${encodeURIComponent(id)}`, {
        method: 'PATCH',
        token,
  ...
  return (
    <div>
      <button type="button" onClick={() => router.push('/admin/channels')} className="text-sm text-sky-400">
        ← Channels
      </button>
      <h1 className="mt-4 text-xl font-bold text-white">{row.name}</h1>
      <p className="text-xs text-slate-500">Network {row.networkId} · members {row.memberCount ?? '—'}</p>
